...
/**
 * ==========================================
 * ESTADO DE DATOS Y CONFIGURACIÓN DEL SISTEMA - BOMBEROS
 * ==========================================
 * Almacenamiento en memoria para simular una base de datos local de bomberos.
 */

// Información del bombero logueado (Diseño rojo oscuro sólido, sin degradados)
const BOMBERO_LOGUEADO = {
  ...
  tipoSangre: "O+",
  estado: "Online",
  fotoUrl: "https://images.unsplash.com/photo-1618015358954-115ef1ed1751?auto=format&fit=crop&q=80&w=256&h=256" // Foto tipo credencial
};
...
const REPORTES_MOCK = [
  {
    ...
    evidencia: true,
    fotoEvidencia: 'https://images.unsplash.com/photo-1508847154043-be12a3283a0b?auto=format&fit=crop&q=80&w=600', // Foto de la escena
    ciudadano: {
      id: 'CC 1.018.472.938',
  ...
  },
  {
    ...
    accionPolicia: '—',
    evidencia: true,
    fotoEvidencia: 'https://images.unsplash.com/photo-1506015391300-4802dc74de2e?auto=format&fit=crop&q=80&w=600', // Foto del área asegurada
    ciudadano: {
      id: 'CC 79.847.291',
  ...
  },
  {
    ...
    accionPolicia: '—',
    evidencia: true,
    fotoEvidencia: 'https://images.unsplash.com/photo-1555854877-bab0e564b8d5?auto=format&fit=crop&q=80&w=600', // Foto de la cerradura
    ciudadano: {
      id: 'CC 52.987.342',
  ...
  },
  {
    ...
    accionPolicia: '—',
    evidencia: true,
    fotoEvidencia: 'https://images.unsplash.com/photo-1568605117036-5fe5e7bab0b7?auto=format&fit=crop&q=80&w=600', // Foto del derrame
    ciudadano: {
      id: 'CC 80.124.957',
  ...
  },
  {
    ...
    accionPolicia: '—',
    evidencia: true,
    fotoEvidencia: 'https://images.unsplash.com/photo-1589829545856-d10d557cf95f?auto=format&fit=crop&q=80&w=600', // Foto del cilindro
    ciudadano: {
      id: 'CC 1.020.448.917',
...
async function cargarReportesDesdeAPI() {
  ...
  /*
  try {
    const respuesta = await fetch('https://api.tu-servidor.com/v1/bomberos/reportes');
    ...
    console.error("Error consumiendo la API de Bomberos, cargando datos locales (Mock):", error);
  }
  */
        ...
        <div>
          ...
          <div class="m-4 p-4 rounded-md bg-[#2a0808] border border-[#ef4444]/25 relative overflow-hidden">
            ...
            <div class="flex items-center justify-between">
              <div class="relative">
                <img src="${BOMBERO_LOGUEADO.fotoUrl}" alt="Oficial" class="w-11 h-11 object-cover rounded-full border border-[#ef4444]/40">
                <span class="absolute bottom-0 right-0 w-2.5 h-2.5 rounded-full bg-[#ef4444] border-2 border-[#2a0808]"></span>
              </div>
              <div class="min-w-0 flex-1 ml-3">
                <div class="text-[8px] uppercase tracking-widest text-[#ef4444] font-bold font-mono">USUARIO</div>
                <div class="text-xs font-semibold text-zinc-100 truncate">${BOMBERO_LOGUEADO.nombre}</div>
            ...
            <div class="xl:col-span-4 flex flex-col gap-6 justify-between">
              ...
              <div class="bg-white rounded-lg p-6 border border-zinc-200 flex flex-col justify-between h-[138px]">
                <div class="flex justify-between items-start">
                  ...
                  <span class="inline-flex items-center gap-1 bg-red-50 border border-red-200 text-[#dc2626] text-[8px] font-bold uppercase font-mono px-2 py-0.5 rounded-md animate-pulse">
                    ● En vivo
                  </span>
                </div>
        ...
        <div id="view-mapa" class="hidden grid grid-cols-1 xl:grid-cols-12 gap-6 items-stretch w-full">
          ...
          <div class="xl:col-span-8 bg-white rounded-lg p-5 border border-zinc-200 flex flex-col justify-between h-[650px] relative">
            ...
            <div class="flex items-center justify-between z-20 absolute bottom-8 left-8 right-8 pointer-events-none">
              <div class="bg-[#dc2626] text-white px-5 py-2.5 rounded-md flex flex-col justify-center pointer-events-auto border border-[#dc2626]">
...
function actualizarMarcadoresEnMapa() {
  ...
  reportes.forEach(rep => {
    ...
    if (rep.tipo === 'Incendio') colorClase = 'bg-red-650';
    else if (rep.tipo === 'Fugas y Derrames') colorClase = 'bg-orange-500';
    else if (rep.tipo === 'Rescate y Emergencias') colorClase = 'bg-blue-500';
    ...
    const popupHtml = `
      <div class="p-1 font-sans rounded text-zinc-900" style="min-width: 190px;">
        <div class="flex items-center justify-between border-b border-zinc-150 pb-1.5 mb-1.5">
          <span class="font-bold text-xs uppercase text-[#dc2626]">${rep.tipo}</span>
...
function renderizarTablaHistorial() {
  ...
  tbody.innerHTML = filtrados.map(rep => {
    // Definir los estilos de los tags sin emojis según el tipo de delito de bomberos
    ...
    // ACCIONES (Muestra el nombre del bombero que modifique el estado)
    const accionTexto = rep.accionPolicia || '—';
...
/**
 * Actualiza el estado de un reporte en memoria y registra al bombero responsable
 */
function actualizarEstadoCasoDirecto(id, nuevoEstado) {
  ...
  if (nuevoEstado !== 'Pendiente') {
    caso.accionPolicia = BOMBERO_LOGUEADO.nombre; // Registra a Javier A. Ortega Ruiz
  } else {
    caso.accionPolicia = '—';
  }
...
function abrirModalEvidencia(id) {
  ...
  document.getElementById('modal-titulo-caso').innerHTML = `<span class="text-[#dc2626] font-mono font-bold">EVIDENCIA ${caso.kpId}</span>`;
  ...
  if (btnSalir) {
    btnSalir.addEventListener('click', () => {
      Swal.fire({
        ...
        text: "Saldrás de la consola táctica de bomberos.",
        icon: 'warning',
